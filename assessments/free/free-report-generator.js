...
class AdvancedReportGenerator {
  ...
  async generateReport(assessmentData) {
    ...
    const report = {
      ...
      archetype: {
        name: enhancedAnalysis?.archetype?.name || this.determinePersonalityArchetype(traits),
        description:
          this.archetypes[
            enhancedAnalysis?.archetype?.name || this.determinePersonalityArchetype(traits)
          ],
        frequency: this.getArchetypeFrequency(
          enhancedAnalysis?.archetype?.name || this.determinePersonalityArchetype(traits)
        )
      },
  ...
  detectNeurodiversityMarkers(responses, responseAnalysis) {
    const markers = {
      adhd: this.screenForADHD(responses),
      autism: this.screenForAutism(responses),
  ...
  screenForADHD(responses) {
    ...
    const impulsivityQuestions = responses.filter(
      r => r.subcategory === 'executive_function' || r.trait === 'conscientiousness'
    );

    const lowConscientiousness = impulsivityQuestions.filter(r => r.score <= 2).length;
    if (lowConscientiousness > impulsivityQuestions.length * 0.6) {
      indicators.score += 30;
      indicators.markers.push('Executive function challenges');
    }
    ...
    if (avgResponseTime < 3000) {
      indicators.score += 20;
      indicators.markers.push('Quick response pattern');
    }
  ...
  screenForAutism(responses) {
    ...
    // Check for pattern-focused responses
    const socialQuestions = responses.filter(
      r => r.trait === 'extraversion' || r.trait === 'agreeableness'
    );

    const lowSocialScores = socialQuestions.filter(r => r.score <= 2).length;
    if (lowSocialScores > socialQuestions.length * 0.5) {
      indicators.score += 25;
      indicators.markers.push('Social interaction differences');
    }

    // Check for high systemizing (high conscientiousness + low extraversion)
    const systematizing = responses.filter(
      r => r.trait === 'conscientiousness' && r.score >= 4
    ).length;
    if (systematizing > responses.length * 0.3) {
      indicators.score += 20;
      indicators.markers.push('Strong systematizing tendency');
    }
  ...
  determinePersonalityArchetype(traits) {
    ...
    if (agreeableness >= 60 && extraversion >= 55 && openness >= 50) {
      return 'Inspirational Connector';
    }
  ...
  generatePersonalNarrative(traits, archetype) {
    ...
    const archetypeName =
      typeof archetype === 'string' ? archetype : archetype.name || 'Unique Individual';

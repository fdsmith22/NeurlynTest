...
class ErrorTracker extends EventEmitter {
    ...
    categorizeError(errorInfo) {
        ...
        // Database errors
        if (type === 'MongoError' || type === 'MongooseError' ||
            message.includes('database') || message.includes('MongoDB')) {
            return 'database';
        }
        ...
        // Validation errors
        if (type === 'ValidationError' || code === 400 ||
    ...
    determineSeverity(error) {
        ...
        // Critical errors
        if (error.code === 500 || error.message.includes('FATAL') ||
            error.message.includes('CRITICAL')) {
            return 'critical';
        }

        // High severity
        if (error.code >= 500 || error.name === 'TypeError' ||
    ...
    sendAlert(alert) {
        ...
        console.error(`   ${alert.count || alert.errors.length} errors in ${alert.timeWindow || 'recent'}`);
    ...
    analyzeErrorTrends() {
        ...
        for (const [category, errors] of Object.entries(this.errors)) {
            const recentErrors = errors.filter(
                err => new Date(err.timestamp) > new Date(now - windowMs)
            );
    ...
    clearOldErrors(olderThanHours = 24) {
        ...
        for (const [category, errors] of Object.entries(this.errors)) {
            this.errors[category] = errors.filter(
                err => new Date(err.timestamp) > new Date(cutoff)
            );
        }

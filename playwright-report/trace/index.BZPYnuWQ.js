...
const M = ({
    ...
    return (
      ...
      r.useLayoutEffect(() => {
        g.current && (s ? (a ? g.current.showModal() : g.current.show()) : g.current.close());
      }, [s, a]),
      e.jsx('dialog', { ref: g, style: j, className: n, 'data-testid': p, children: T })
    );
  },
  G = (n, o) => {
    const s = U(n, o, 'left');
  ...
  H = () => {
    ...
    return e.jsxs(e.Fragment, {
      children: [
        e.jsx(z, { ref: n, icon: 'settings-gear', title: 'Settings', onClick: () => s(a => !a) }),
        e.jsx(M, {
    ...
    (r.useEffect(() => {
      ...
      r.useEffect(() => {
        const t = c => {
          ...
          if (d !== 'load' || !((i == null ? void 0 : i.trace) instanceof Blob)) return;
          const v = new File([i.trace], 'trace.zip', { type: 'application/zip' }),
            f = new DataTransfer();
      ...
      r.useEffect(() => {
        (async () => {
          if (s.length) {
            ...
            (navigator.serviceWorker.addEventListener('message', t), T({ done: 0, total: 1 }));
            const c = [];
      ...
      children: [
            ...
            children: [
              ...
              e.jsx('div', {
                ...
                children: e.jsx('div', {
                  ...
                  style: { width: p.total ? (100 * p.done) / p.total + '%' : 0 }
                })
              })
            ]
          })
        ...
        R &&
          e.jsxs('div', {
            className: 'drop-target',
            children: [
              e.jsx('div', { className: 'processing-error', role: 'alert', children: b }),
              e.jsx('div', {
                className: 'title',
                role: 'heading',
                'aria-level': 1,
...
(async () => {
  ...
  if ((V(), window.location.protocol !== 'file:')) {
    if (
      (n.get('isUnderTest') === 'true' && (await new Promise(l => setTimeout(l, 1e3))),
...
Make sure to serve the Trace Viewer (${window.location}) via HTTPS or localhost.`);
  ...
  const o = n.get('trace'),
    a = (o == null ? void 0 : o.endsWith('.json')) ? e.jsx(_, { traceJson: o }) : e.jsx(K, {});
